...
export class SettingsHandler {
  ...
  private async setConfiguration(settings: Settings): Promise<void> {
    ...
    if (settings.yaml) {
      ...
      if (settings.yaml.crdStore) {
        this.yamlSettings.crdStoreEnabled = settings.yaml.crdStore.enable;
        if (settings.yaml.crdStore.url?.length !== 0) {
          this.yamlSettings.crdStoreUrl = settings.yaml.crdStore.url;
        }
      }
